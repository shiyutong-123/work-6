...
trait IssuesControllerBase extends ControllerBase {
  ...
  private case class CommentForm(issueId: Int, content: String, featureToggle: Boolean)
  ...
  private val commentForm = mapping(
    ...
    "featureToggle" -> label("Feature Toggle", boolean())
  )(CommentForm.apply)
  ...
  post("/:owner/:repository/issue_comments/new", commentForm)(readableUsersOnly { (form, repository) =>
    context.withLoginAccount { loginAccount =>
      getIssue(repository.owner, repository.name, form.issueId.toString).flatMap { issue =>
        ...
        handleComment(issue, Some(form.content), repository, actionOpt, form.featureToggle) map { case (issue, id) =>
          redirect(
    ...
    context.withLoginAccount { loginAccount =>
      getComment(repository.owner, repository.name, params("id")).map { comment =>
        if (isEditableContent(repository.owner, repository.name, comment.commentedUserName, loginAccount)) {
          updateComment(repository.owner, repository.name, comment.issueId, comment.commentId, form.content, form.featureToggle)
